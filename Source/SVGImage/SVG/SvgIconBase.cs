...
namespace SVGImage.SVG
{
    ...
    public abstract class SvgIconBase : MarkupExtension
    {
        ...
        protected virtual DrawingGroup GetDrawing(Uri svgSource)
        {
            ...
            switch (scheme)
            {
                ...
                case "https":
                case "http":
                    using (FileSvgReader reader = new FileSvgReader(this.OverrideFill, this.OverrideBrush))
                    {
                        ...
                        && string.Equals(sEncoding.Trim(), "base64", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (isGZiped)
                        {
                            using (var stream = new MemoryStream(imageBytes))
                            {
                                ...
                                {
                                    using (var reader = new FileSvgReader(this.OverrideFill, this.OverrideBrush))
                                    {
                                        DrawingGroup drawGroup = reader.Read(zipStream);
                                        if (drawGroup != null)
                                        {
    ...
    internal sealed class FileSvgReader : IDisposable
    {
        ...
        public FileSvgReader(Color? overrideFill, Color? overrideStroke)
        {
